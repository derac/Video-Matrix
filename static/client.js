...
[...document.getElementsByTagName('*')].forEach((t) => {
  t.onfocus = (e) => {
    e.target.blur();
  };
}); // prevent focus on all elements except
['search_box', 'volume', 'results'].forEach((n) => {
  document.getElementById(n).onfocus = () => {};
}); // these

if (is_touchscreen()) {
  // change styling and other settings to improve touch handling
  document.getElementById(
    'touch_style'
  ).innerHTML = `.material-icons { font-size: 8vmin; } .button-group input, .button-group button { width: 16vmin; height: 16vmin; }
...
#search_box { font-size: 12vmin; padding: 1vmax; padding-left: 2vmax; }`;
  ...
  vids[0].onfocus = () => {};
  if (localStorage.controls_on === undefined) {
    localStorage.controls_on = true;
  }
} // if we're on a touch device, show controls
else if (localStorage.controls_on === undefined) {
  localStorage.controls_on = false;
} // if not, hide them by default
if (localStorage.help_on === undefined) {
...
if (hd == 1) {
  set_hd();
} // more control state defaults

if (!JSON.parse(localStorage.controls_on)) {
  toggle_controls(true);
}
update_url(); // set controls to proper state and update url
if (!JSON.parse(localStorage.help_on)) {
  toggle_help(true);
} // set help to proper state
if (vid_count < 1) {
  vid_count = 1;
  update_url();
} else {
...
set_volume(0.5);
document.getElementById('volume').value = 0.5; // set initial volume to half
...
function update_url() {
  history.pushState(
    {},
    'Video Matrix',
    `?n=${vid_count}&results=${results}&hd=${hd}&search=${search}`
  );
...
function get_stretch_grid(n) {
  // utility function to get a list of [x,y,w,h] for sub elements in percentages, swap x/y w/h if is_wide is true
  (cols = Math.floor(Math.sqrt(Math.ceil((n + 1) / (1 + !is_wide))))),
    (w = 100 / cols),
    (Grid = []);
  for (c = 0; c < cols; c++) {
    x = c * w;
...
function load_video(i) {
  ...
  fetch(
    '/search/' +
      (search == 'default'
        ? search_list[Math.floor(Math.random() * search_list.length)]
        : search) +
      '?results=' +
      results +
      '&hd=' +
    ...
    .then((data) => {
      ...
      direct_urls[i] = data;
      v.play().then(() => {
        v.currentTime = Math.max(v.duration / 3, 600);
      });
    })
    .catch((error) => {
      console.error(error);
...
function add_video() {
  ...
  if (vid_count > vids.length) {
    ...
    new_vidc.getElementsByTagName('button')[0].onfocus = (e) => {
      e.target.blur();
    }; // prevent focus
    if (!is_touchscreen()) {
      new_vid.onfocus = (e) => {
    ...
    // else if vidc is already added, put the url we've cached in it, then show the ones we need
    new_vid = vid_els[vid_count - 1].getElementsByTagName('video')[0];
    new_vid.src = direct_urls[vid_count - 1];
    new_vid.play().then(() => {
      new_vid.currentTime = Math.max(new_vid.duration / 3, 600);
    });
  }
  update_grid();
}
...
function update_grid() {
  get_stretch_grid(vid_count).forEach((P, i) => {
    vid_els[
      i
    ].style = `left:${P[0]}%;top:${P[1]}%;width:${P[2]}%;height:${P[3]}%;display:block;`;
  });
}
...
function change_volume(change) {
  document.getElementById('volume').value = value =
    parseFloat(document.getElementById('volume').value) + change;
  set_volume(value);
...
function set_hd() {
  document.getElementById('hd').innerHTML = hd
    ...
    : '<i class="material-icons" style="color:#40453a">hd</i>';
}

function toggle_controls(old_state) {
  document.getElementById('controls').style.display = old_state
    ? 'none'
    : 'table';
...
function toggle_mute(old_state) {
  ...
  document.getElementById('toggle_mute').innerHTML = old_state
...
function toggle_pause(old_state) {
  vids.forEach((v) => {
    old_state ? v.play() : v.pause();
  });
  is_paused = !is_paused;
  document.getElementById('toggle_pause').innerHTML = old_state
...
function toggle_search() {
  search_on = !search_on;
  document.getElementById('search').style.display = search_on
    ? 'block'
    : 'none';
  if (search_on) {
...
}; // remove search on back
window.onresize = () => {
  if (window.innerWidth > window.innerHeight != is_wide) {
    is_wide = !is_wide;
    update_grid();
  } // update the grid and toggle_fullscreen button onresize
  document.getElementById('toggle_fullscreen').innerHTML =
    '<i class="material-icons">' +
    (window.innerHeight == screen.height ? 'fullscreen_exit' : 'fullscreen') +
    '</i>';
  if (is_touchscreen()) {
    document.getElementById('touch_style_fullscreen').innerHTML =
      window.innerHeight == screen.height ? '#search {top: 25%;}' : '';
  }
};
  ...
  if (!e.repeat) {
    if (!search_on) {
      if (!isNaN(e.key)) {
        ...
        s = JSON.parse(localStorage.controls_on);
        bc.postMessage(['toggle_controls', s]);
        toggle_controls(s);
      } else if (k == 'h') {
        s = JSON.parse(localStorage.help_on);
        bc.postMessage(['toggle_help', s]);
        toggle_help(s);
      } else if (k == 'p') {
        bc.postMessage(['toggle_pause', is_paused]);
        toggle_pause(is_paused);
...
bc.onmessage = (e) => {
  d = e.data; // broadcasting/multi-window support
  if (d == 'reload') {
    reload_all();
  } else if (d == 'forward') {
    forward();
  ...
  } else if (d[0] == 'toggle_pause') {
    toggle_pause(d[1]);
  } else if (d[0] == 'toggle_mute') {
    toggle_mute(d[1]);
  } else if (d[0] == 'toggle_controls') {
    toggle_controls(d[1]);
  } else if (d[0] == 'toggle_help') {
    toggle_help(d[1]);
  } else if (d[0] == 'set_volume') {
    document.getElementById('volume').value = d[1];
    set_volume(d[1]);
  }
};
